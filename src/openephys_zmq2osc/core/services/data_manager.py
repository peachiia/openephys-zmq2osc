...
class DataManager:
    def __init__(self):
        self.channels = []
        self.buffer_size = (
            30000  # 1 Second of data at 30kHz sample rate (minimal buffering)
        )
    ...
    def init_empty_buffer(self, num_channels: int, num_samples: int) -> None:
        ...
        if num_channels <= 0 or num_samples <= 0:
            raise ValueError(
                "Number of channels and samples must be positive integers."
            )
        ...
        print(
            f"Initialized empty buffer with {num_channels} channels and {num_samples} samples per channel."
        )
    ...
    def pop_data(self, channel_id: int, num_samples_to_pop: int) -> np.ndarray:
        ...
        if num_samples_to_pop <= 0 or num_samples_to_pop > (
            tail_sample_number - head_sample_number
        ):
    ...
    def pop_data_all_channels(self, num_samples_to_pop: int) -> list[np.ndarray]:
        ...
        for channel in self.channels:
            if (
                channel["tail_sample_number"] - channel["head_sample_number"]
                < num_samples_to_pop
            ):
                raise ValueError(
                    f"Not enough data in channel {channel['id']} to pop {num_samples_to_pop} samples."
                )
    ...
    def add_or_expand_channel(self, channel_id: int, channel_name: str = None) -> bool:
        ...
        if channel_id in self.discovered_channels:
            ...
            if self.channel_discovery_mode:
                print(
                    f"Channel discovery complete! Detected {len(self.discovered_channels)} channels."
                )
        ...
        while len(self.channels) < required_channels:
            ...
            label = (
                channel_name
                if (new_id == channel_id and channel_name)
                else f"CH{new_id}"
            )
        ...
        print(
            f"Discovered channel {channel_id} ({channel_name or f'CH{channel_id}'}). Total: {len(self.discovered_channels)} channels"
        )
        return False
    ...
    def configure_timeout(
        self, timeout_seconds: float, auto_reinit: bool = False
    ) -> None:
    ...
    def reinit_for_new_setup(self) -> dict:
        ...
        print(
            f"DataManager reinitialized: {previous_state['total_channels']} → 1 channel"
        )
